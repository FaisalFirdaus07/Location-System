...
// --- KONFIGURASI WIFI ---
#define WIFI_TIMEOUT_MS     10000   // Batas waktu koneksi awal: 10 detik
#define WIFI_RETRY_DELAY_MS 5000    // Jeda antar percobaan reconnect: 5 detik
#define WIFI_MAX_RETRY      5       // Maksimal percobaan reconnect sebelum restart WiFi
...
bool ensureWiFiConnected() {
  ...
  // Jika semua retry gagal, reset WiFi sepenuhnya
  Serial.println("[WiFi] ❌ Semua percobaan gagal. Mereset WiFi...");
  WiFi.disconnect(true);
  WiFi.mode(WIFI_OFF);
  vTaskDelay(1000 / portTICK_PERIOD_MS);
  return connectWiFi(); // Satu percobaan terakhir setelah reset penuh
}
...
void TaskUpload(void *pvParameters) {
  ...
  while (true) {
    if (millis() - lastUpload >= UPLOAD_INTERVAL) {
      ...
      // Pastikan WiFi terhubung sebelum upload, reconnect jika perlu
...
void setup() {
  ...
  // Koneksi WiFi awal dengan blocking + timeout (tidak lagi fire-and-forget)
  Serial.println("\n--- Memulai Koneksi WiFi ---");
  connectWiFi(); // Coba koneksi pertama, TaskUpload akan handle reconnect jika gagal
